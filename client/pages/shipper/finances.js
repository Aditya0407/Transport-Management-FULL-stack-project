...
const financeStyles = {
  ...
  balanceAmount: {
    fontSize: '28px',
    fontWeight: 'bold',
    color: '#dc3545', // Red for payments (shipper perspective)
  },
  balanceLabel: {
    fontSize: '16px',
    color: '#6c757d',
...
export default function ShipperFinances() {
    ...
    <div style={financeStyles.container}>
      ...
      {authState.user && (
        <div style={financeStyles.balanceCard}>
          ...
          <div>
            <button style={financeStyles.paymentButton}>
              Add Payment Method
            </button>
          </div>
        </div>
      ...
      <div style={financeStyles.summary}>
        <div style={financeStyles.summaryCard}>
          <div style={financeStyles.summaryTitle}>Total Payments</div>
          <div style={financeStyles.summaryValue}>{formatCurrency(summary.totalPayments)}</div>
        </div>
        <div style={financeStyles.summaryCard}>
        ...
        <div style={financeStyles.summaryCard}>
          ...
          <div style={financeStyles.summaryValue}>{formatCurrency(summary.thisMonthPayments)}</div>
        </div>
      </div>
            ...
            <table style={financeStyles.table}>
              <thead>
                <tr>
                  ...
                  <th style={financeStyles.th}>Load ID</th>
                  <th style={financeStyles.th}>Trucker</th>
                </tr>
              </thead>
              <tbody>
                {transactions.map(transaction => (
                  <tr key={transaction._id}>
                    ...
                    <td style={financeStyles.td}>
                      {transaction.load ? (
                        <a href={`/shipper/load-details?id=${transaction.load}`} style={{ color: '#007bff' }}>
                          {transaction.load.substring(0, 8)}...
                        </a>
                      ) : 'N/A'}
                    </td>
                    <td style={financeStyles.td}>{transaction.trucker ? transaction.trucker.name : 'N/A'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div style={financeStyles.section}>
        <h2 style={financeStyles.sectionTitle}>Payment Methods</h2>
        <div style={financeStyles.card}>
